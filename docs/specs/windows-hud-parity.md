...
- **IDLE** (per [#56](https://github.com/Hanimn/seda/issues/56)): a **compressed pill** — a short horizontal capsule (28×6, cap radius = h/2) at panel centre — with a faint slow shimmer as an **alpha pulse** (`shimmer = 0.55 + 0.15·sin(phase·2π/2.6)`, i.e. a ~2.6 s breath floored at 0.4). These are the #56 prototype-verified constants, defined once as shared knobs in `seda/notifications/__init__.py` (ADR-0007 §5) and imported by both hosts. Under Option B this is a real per-pixel alpha pulse (no gray-value substitution). Waking widens the pill back into the 9 bars. Both hosts shrink the panel to 48×24 (macOS resizes the `NSPanel`; Windows blits only the top-left 48×24 sub-rect of the fixed 160×48 DIB — see Part 4), recentred on the active footprint, at the ~10 Hz idle cadence.
...
- **Panel-shrink (as shipped, #79):** no `SetWindowPos`. Under Option B the `UpdateLayeredWindow` `psize` **is** the window size, so a single ULW call at the new dimensions resizes, repositions, and repaints atomically — strictly stronger than a `SetWindowPos`+re-blit pair (nothing to leave un-dimension-matched, so it cannot tear for a frame). The DIB stays a fixed 160×48; IDLE blits only its top-left 48×24 sub-rect (`pptSrc = (0,0)`, `psize = (48,24)`), recentred on the active footprint via `pptDst`. `set_mode` computes the mode footprint once (`_mode_size`) and feeds the *same* tuple to `_paint` (which sub-rect to draw) and `_blit` (`psize`/`pptDst`) in one immediate tick — draw-size and blit-size can never disagree. No focus steal, topmost band untouched (`WS_EX_*` unchanged).
