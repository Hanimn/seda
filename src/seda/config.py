...
# The project was renamed from "Local Flow" to "Seda"; the config/cache
# directory moved with it. Kept so we can point users who still have config
# under the old name to the new location (see :func:`migration_notice`).
OLD_APP_NAME = "local-flow"
...
class Config(_Section):
    """The complete, validated Seda configuration."""
...
def default_config_path() -> Path:
    ...
    ``~/Library/Application Support/seda`` on macOS,
    ``~/.config/seda`` on Linux, and ``%APPDATA%\\seda`` on
    Windows (see §11).
    """
...
def migration_notice() -> str | None:
    """Return a one-time notice if config still lives under the old app name.

    After the Local Flow → Seda rename the config/cache directory moved from
    ``<config>/local-flow`` to ``<config>/seda``. When the new directory does
    not exist yet but the old one does, return a human-readable message telling
    the user where their old config is and how to migrate it. We do **not**
    auto-copy — moving a user's config file for them is riskier than telling
    them where it is.

    Returns ``None`` when there is nothing to say (new dir already present, or
    no old dir), or if the paths cannot be resolved (best-effort; never raises).
    """
    try:
        new_dir = user_config_path(APP_NAME, appauthor=False)
        old_dir = user_config_path(OLD_APP_NAME, appauthor=False)
        if new_dir.exists() or not old_dir.exists():
            return None
        return (
            f"found configuration under the old '{OLD_APP_NAME}' name at {old_dir}. "
            f"Seda now uses {new_dir} — move your config there "
            f"(e.g. `mv {old_dir} {new_dir}`), or run `seda config init` to start fresh."
        )
    except Exception:  # noqa: BLE001 - best-effort; a probe failure is not fatal
        return None
