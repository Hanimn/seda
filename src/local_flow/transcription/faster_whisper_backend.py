...
from local_flow.config import TranscriptionConfig
from local_flow.errors import ModelUnavailableError, TranscriptionError
from local_flow.logging_config import get_logger
    ...
    def load(self) -> None:
        ...
        if self._offline and _looks_like_missing_model(last_error):
            raise ModelUnavailableError(
                ...
                "forbids downloading it; run `local-flow models download` first "
                f"(cause: {message})"
            ) from last_error
        raise ModelUnavailableError(
